...
std::array<tile_uid_t, WorldMatrix::MAX_TILES_PER_POS>& WorldMatrix::operator[](const SafeVec coords)
{
    return flattenedUidsMatrix[SIZE.lef*coords.lef + coords.RIGHT];
}
std::array<tile_uid_t, WorldMatrix::MAX_TILES_PER_POS>& WorldMatrix::at(const SafeVec coords)
{
    return flattenedUidsMatrix.at(SIZE.lef*coords.lef + coords.RIGHT);
}
unsigned char WorldMatrix::countAt(const SafeVec coords)
{
...
void WorldMatrix::resize()
{
    ...
    {
        constexpr static std::array<tile_uid_t, MAX_TILES_PER_POS> ARRAY_OF_NULL_TILES{initialize_uids_array_as_empty<MAX_TILES_PER_POS>()};// DON'T FORGET TO ADD ZEROES IF
