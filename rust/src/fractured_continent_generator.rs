...
{// place_dungeon_entrances
  ...
  while placed_count < placed_dungeons_coords.len() {
      let r_coords = UnsVec {
        ...
        let peninsulerness = peninsuler.get_noise_2d(r_coords);
          ...
          && clear_of(&all_trees, r_coords, 3, true)
        {
            ...
            if placed_dungeons_coords.iter().all(|coord| is_separated_enough(coord)) {
                *placed_dungeons_coords.get_unchecked_mut(placed_count) = r_coords;
                overwrite_formation_tile(world_ptr, (origin, r_coords), unidordist_mapped2targets.drf().get_unchecked(Target::Cave0 as usize + placed_count).get_unid(rng), None);
                placed_count += 1;
