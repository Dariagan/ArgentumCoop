...
fn generate(mut world: WorldMatrix, origin: UnsVec, size: UnsVec,
    ...
    }let continenter_sampling_offset: UnsVec = continenter_sampling_offset;
    ...
    for rel_coords in (hori_range.0..hori_range.1).flat_map(|i| (0..size.right).map(move |j| UnsVec::from((i,j)))){
        ...
        let (peninsulerness, peninsuler_caved) = nv_surpasses_cutoff(peninsuler, rel_coords, PENINSULER_CUTOFF);

        let rng: &mut Lcg128Xsl64 = rngs.drf().get_unchecked_mut(thread_i);
        if continental && peninsuler_caved{
            let (unid, z_level) = nids_mapped_to_targets.drf().get_unchecked(Target::Cont as usize).get_a_nid(rng);
            *tiles_2b_placed.get_unchecked_mut(z_level as usize) = unid
        }
        else {
            let (unid, z_level) = nids_mapped_to_targets.drf().get_unchecked(Target::Cont as usize).get_a_nid(rng);
            *tiles_2b_placed.get_unchecked_mut(z_level as usize) = unid;
...
#[inline]
pub fn get_beachness(rel_coords: UnsVec, big_beacher: SendPtr<FastNoiseLite>, small_beacher: SendPtr<FastNoiseLite>, 
    (continentness, continental_cutoff): (f32, f32), (peninsulerness, peninsuler_cutoff): (f32, f32)) -> f32 {unsafe{
    (0.72 + big_beacher.drf().get_noise_2d(rel_coords.lef as f64, rel_coords.right as f64)/2.3 - f32::powf(continentness-continental_cutoff,0.6))
    .max(0.8 + small_beacher.drf().get_noise_2d(rel_coords.lef as f64, rel_coords.right as f64)/2.3 - f32::powf(peninsulerness-peninsuler_cutoff,0.45))
}}
