...
void ArgentumTileMap::gLoadTilesAround(const Vector2 global_coords, const Vector2i CHUNK_SIZE, const int being_uid)//hacer q el chunk size cambie según el zoom ingame??
{   
...
void ArgentumTileMap::gGenerateWorldMatrix(const Vector2i size, const Dictionary& tiles_data)
{
    if (size.x <= 0 || size.y <= 0){
...
Dictionary ArgentumTileMap::gGetTilesData(){return m_tiles_data;}; 


void ArgentumTileMap::gSetTilesData(const Dictionary& input_tiles_data)
{
    ...
    for(int i = 0; i < m_tiles_data.values().size(); i++)
    {
        ...
        {
            UtilityFunctions::printerr("read gd_tile_data is not valid (ArgentumTileMap.cpp::gSetTilesData())");
            continue;
        }
        ...
        for(int j = 0; j < gd_tile_data.values().size(); j++)
        {
            ...
            if (kgTileFieldKey == ATLAS_ORIGIN_POSITION)
            {
                ...
                {
                    UtilityFunctions::printerr("Atlas origin position ",kgAtlasOriginPosition," is negative for tile_id \"",gd_current_tile_key,"\", using (0,0) (at ArgentumTileMap.cpp::gSetTilesData())");
            ...
            else if (kgTileFieldKey == MODULO_TILING_AREA)
            {
                ...
                {
                    UtilityFunctions::printerr("non-positive MODULO_TILING_AREA ",kgModuloArea," is not admitted. tile_id:",gd_current_tile_key," (at ArgentumTileMap.cpp::gSetTilesData())");
        ...
        {
            UtilityFunctions::printerr("op not found, using (0,0) (at ArgentumTileMap.cpp::gSetTilesData())");
            cppTileData.insert({ATLAS_ORIGIN_POSITION, Vector2i(0, 0)});
        }
        if (cppTileData.count(MODULO_TILING_AREA) == 0)
        {
            UtilityFunctions::printerr("ma not found, using (1,1) (at ArgentumTileMap.cpp::gSetTilesData())");
...
{   
    ...
    ClassDB::bind_method(D_METHOD("generate_world_matrix", "size"), &ArgentumTileMap::gGenerateWorldMatrix);
    
    ClassDB::bind_method(D_METHOD("load_tiles_around", "coords", "chunk_size", "uid"), &ArgentumTileMap::gLoadTilesAround);
    ...
    ClassDB::bind_method(D_METHOD("set_tiles_data", "tiles_data"), &ArgentumTileMap::gSetTilesData);
    ClassDB::bind_method(D_METHOD("get_tiles_data"), &ArgentumTileMap::gGetTilesData);
