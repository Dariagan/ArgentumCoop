...
{
    const Variant& getbeingkinds_result = mArgentumTileMap->mgGlobalData->get("beingkinds");
    if(getbeingkinds_result.get_type() != Variant::DICTIONARY)
    {
        UtilityFunctions::printerr("couldn't get dict beingkinds from mgGlobalData");
        return;
    }
