...
fn generate(mut world: WorldMatrix, origin: UnsVec, size: UnsVec,
  ...
  continenter.frequency = 0.3/size.length_f32().powf(0.995);
  ...
  //-------------THREAD-SPECIFIC-CODE-STARTS HERE------------------
  ...
  let (continentness, continental): (f32, bool) = val_surpasses_cutoff(calc_continentness(continenter, rel_coords, size, None, Some(continenter_offset)), continenter_cutoff);

  let (peninsulerness, peninsular) : (f32, bool) = noise_surpasses_cutoff(peninsuler, rel_coords, PENINSULER_CUTOFF);

  let rng: &mut Lcg128Xsl64 = rngs.drf().get_unchecked_mut(thread_i);
  if continental && peninsular{

    let (beachness, beach) = val_surpasses_cutoff(calc_beachness(rel_coords, big_beacher, small_beacher, (continentness, continenter_cutoff), (peninsulerness, PENINSULER_CUTOFF)), BEACHER_CUTOFF);
    if beach {
    tiles_2b_placed.assign_unid_unchecked(unidordist_mapped2targets.drf().get_unchecked(Target::Beach as usize).get_unid(rng))
      ...
      let lake = away_from_coast && calk_lakeness(rel_coords, (big_laker, BIG_LAKER_CUTOFF), (small_laker, SMALL_LAKER_CUTOFF), beachness);
...
pub fn calc_beachness(rel_coords: UnsVec, big_beacher: SharedNoise, small_beacher: SharedNoise, 
  (continentness, continental_cutoff): (f32, f32), (peninsulerness, peninsuler_cutoff): (f32, f32)) -> f32 {
...
#[inline]
pub fn calk_lakeness(rel_coords: UnsVec, (big_laker, big_laker_cutoff): (SharedNoise, f32), (small_laker, small_laker_cutoff): (SharedNoise, f32), 
  beachness: f32) -> bool {
