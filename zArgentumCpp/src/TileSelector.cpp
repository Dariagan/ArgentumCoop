        ...
        TARGETS_COUNT(((Array)gdTileSelection->get("targets")).size())
    {
        ...
        for (short unsigned int i = 0; i < TARGETS_COUNT; i++)
        {
            ...
            if(((String)gd_tile_to_place[i])[0] != '_')
                TileUidOrGroup[i] = GlobalDataCpp::getTileUid((String)gd_tile_to_place[i]).value_or(std::numeric_limits<uint16_t>::max());
            else
            {
                ...
                {
                    groupedTileUid[j] = GlobalDataCpp::getTileUid(group_dict.keys()[j]).value_or(std::numeric_limits<uint16_t>::max());
    ...
    uint16_t getTileUidForTarget(const char* inputTargetTofill)
    {
        ...
        if (it != AvailableTargets.end())
        {
            ...
            try
            {
                ...
                return optTileID.value_or(NULL_TILE_UID);
            }
