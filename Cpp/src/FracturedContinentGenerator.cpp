...
double FracturedContinentGenerator::getContinentness(SafeVec coords) const
{
    const double BCF = FormationGenerator::getBorderClosenessFactor(coords, mSize);

    return mContinenter.get_noise_2dv(coords) * (1-BCF);
}
double FracturedContinentGenerator::getBeachness(SafeVec coords) const
{
    return std::max(
...
void FracturedContinentGenerator::placeDungeonEntrances(
    godot::ArgentumTileMap& argentumTileMap, std::uint_fast8_t dungeonsToPlace)
{
...
double FracturedContinentGenerator::gGetContinentalCutoff()const{return mContinentalCutoff;}
void FracturedContinentGenerator::gSetContinentalCutoff(double cutoff){mContinentalCutoff = cutoff;} 
void FracturedContinentGenerator::_bind_methods()
{
    ClassDB::bind_method(D_METHOD("set_continental_cutoff", "continental_cutoff"), &FracturedContinentGenerator::gSetContinentalCutoff);
    ClassDB::bind_method(D_METHOD("get_continental_cutoff"), &FracturedContinentGenerator::gGetContinentalCutoff);
