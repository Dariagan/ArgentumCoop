...
std::array<tiletype_uid, WorldMatrix::MAX_TILES_PER_POS>& WorldMatrix::operator[](const SafeVec coords)
{
    return flattenedUidsMatrix[SIZE.lef*coords.lef + coords.RIGHT];
}
std::array<tiletype_uid, WorldMatrix::MAX_TILES_PER_POS>& WorldMatrix::at(const SafeVec coords)
{
...
void WorldMatrix::resize()
{
    ...
    {
        constexpr static std::array<tiletype_uid, MAX_TILES_PER_POS> ARRAY_OF_NULL_TILES{initialize_uids_array_as_empty<MAX_TILES_PER_POS>()};// DON'T FORGET TO ADD ZEROES IF
